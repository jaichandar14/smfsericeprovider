...
internal class ActionDetailsRepositoryTest {

    @get:Rule
    var instantExecutorRule: InstantTaskExecutorRule = InstantTaskExecutorRule()

    private lateinit var repository: ActionDetailsRepository
    ...
    fun setUp() {
        ...
        repository = ActionDetailsRepository(testApis)
    }
    ...
    @Test
    fun `for postQuote details, api must return postQuote details with http code 200`() =
        runBlocking {
    ...
    @Test
    fun `for getBidActions details, api must return getBidActions details with http code 200`() =
        runBlocking {
            val newRequestList = NewRequestList(
                Data(0.0, 0.0, 0.0, null, 0.0),
                Result("success"),
            ...
            when (val actualResponse = repository.getBidActions("", 0, 0, 0, "")) {
                is ApisResponse.Success -> {
                    Truth.assertThat(actualResponse.response).isEqualTo(newRequestList)
                }
                else -> {}
    ...
    @Test
    fun `for server error, getBidActions details api must return with http error message`() =
        runBlocking {
            val expectedResponse = MockResponse()
                .setResponseCode(HttpURLConnection.HTTP_INTERNAL_ERROR)
                .setBody(Gson().toJson(null))
